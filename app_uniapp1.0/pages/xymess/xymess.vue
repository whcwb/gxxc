		...
		methods: {
			...
			getZYmess(){//获取专员信息
			 ...
			 getHandleStatus(){// 获取受理状态信息
			  this.$http.post(this.apis.getHandleStatus,{yhId: this.usermess.yhId}).then((res)=>{
			...
			getPayInfo() {// 缴费信息
			  this.$http.post(this.apis.getPayInfo,{yhId: this.usermess.yhId}).then((res)=>{
				if (res.code == 200 && res.result) {
			...
			getExamInfo() {// 考试信息
			  let v = this;
			  this.$http.get(this.apis.getExamInfo,{yhId: this.usermess.yhId}).then((res)=>{

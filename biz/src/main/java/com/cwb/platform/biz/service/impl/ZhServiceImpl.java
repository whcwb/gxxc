        ...
        ids.forEach(
                s ->  {
                    ...
                    if(CollectionUtils.isNotEmpty(ptyhs)){
                        // 区分已经考过科目一 和 没考过的学员
                        List<String> collect = ptyhs.stream().filter(bizPtyh -> StringUtils.equals(bizPtyh.getYhLx(),
                        ...
                        // 查询用户下面的二级用户 , 准备二级分佣
                        List<String> yqms = ptyhs.stream().map(BizPtyh::getYhZsyqm).collect(Collectors.toList());
                        ...
                        List<String> collect1 = rPtyhs.stream().map(BizPtyh::getId).collect(Collectors.toList());

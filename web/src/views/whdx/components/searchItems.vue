<template>
    <span  :id="'searchRow'">
        <span v-for="(r,index) in tableColumns" v-if="r.searchKey || r.searchType" :key="index">
            ...
            <Input v-if="!r.searchType || r.searchType == 'text'" v-model="form[r.searchKey]" :placeholder="'请输入'+r.title" :style="inputStyle" clearable @on-change="parent.util.getPageData(parent)"></Input>
            ...
            <Select v-else-if="r.searchType == 'dict'" filterable clearable  v-model="form[r.key]" :placeholder="'不限'" :style="inputStyle" @on-change="parent.util.getPageData(parent)" >
